...
class Brick: Comparable {
  ...
  static func < (lhs: Brick, rhs: Brick) -> Bool {
    lhs.range.lowerBound.z < rhs.range.upperBound.z
  }
  ...
  static func part2(bricks: [FallenBrick]) -> Int {
    return bricks.map{
      ...
      while !Q.isEmpty {
        ...
        for s in curr.supporting {
          if !displacedBricks.contains(s) &&
            displacedBricks.isSuperset(of: s.standingOn) {
            displacedBricks.insert(s)
            Q.append(s)
          }
        }
      }
